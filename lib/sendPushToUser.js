 ...
 * @param {string} userId
 * @param {{title: string, body: string, url?: string, channelId?: "customer_channel" | "agent_channel"}} payload
 * @returns {Promise<number>} عدد الإشعارات اللي وصلت بنجاح
 */
export async function sendPushToUser(
  supabaseAdmin,
  userId,
  { title, body, url = "/", channelId = "customer_channel" }
) {
  let sent = 0;
  ...
  const fcmResults = await Promise.allSettled(
    (fcmRows || []).map((row) =>
      admin.messaging().send({
        token: row.token,
        // مهم: بنبعت data بس من غير notification، عشان نظام أندرويد
        // متعرضش الإشعار تلقائي بنفسه - وبدل كده كود الأندرويد بتاعنا
        // (MyFirebaseMessagingService) هو اللي بيقرر يعمل إيه بالظبط
        // حسب حالة التطبيق (فاتح / خلفية)
        data: {
          title: title || "",
          body: body || "",
          url,
          channelId,
          // معرف فريد لكل رسالة، عشان أي إشعار جديد يتعامل معاه النظام
          // كإشعار مستقل تمامًا، مش تحديث لإشعار سابق
          notificationId: `${Date.now()}-${Math.floor(Math.random() * 100000)}`,
        },
